...
// GET /api/icons — list icons (any authenticated user)
export async function GET(request) {
  try {
    await checkUserAuthWithRole(request, []);

    const { search } = new URL(request.url);
    const { data, status } = await axios.get(`${BACKEND_URL}/icons${search}`, {
...
// POST /api/icons — create icon (admin or superadmin)
export async function POST(request) {
  try {
    await checkUserAuthWithRole(request, ["admin", "superadmin"]);

    const body = await request.json();
    const { data, status } = await axios.post(`${BACKEND_URL}/icons`, body, {
